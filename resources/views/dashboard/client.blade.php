...
@section('sidebar')
    ...
    <x-sidebar-label label="Mon activité" />
    <div class="grid grid-cols-2 gap-1.5">
        <x-metric-card label="Favoris"   :value="$stats['favorites']"    sub="sauvegardés" />
        <x-metric-card label="Visites"   :value="$stats['appointments']" sub="demandées" />
        <x-metric-card label="Messages"  :value="$stats['messages']"     sub="envoyés" />
        <x-metric-card label="Consultés" :value="$stats['viewed']"       sub="biens vus" />
    </div>

    <x-sidebar-label label="Navigation" />
    ...
    <x-sidebar-label label="Consultés récemment" />
    @forelse($recentlyViewed as $property)
    <div class="py-1.5 border-b border-dark-border/40 last:border-0">
        <a href="{{ route('properties.show', $property) }}" class="text-[11px] text-dark-muted hover:text-dark-text transition-colors">
...
<div class="p-3 flex flex-col gap-3">
    ...
    {{-- Favoris --}}
    <div>
        <div class="text-[9px] tracking-[.15em] text-dark-dim uppercase mb-2">Mes favoris</div>
        <div class="grid grid-cols-2 gap-2">
            ...
            <div class="bg-dark-card2 border border-dark-border rounded-sm overflow-hidden">
                ...
                <div class="w-full h-24 bg-dark-card3 flex items-center justify-center">
                    <svg class="w-6 h-6 text-dark-dim" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="1"><path d="M1 9V5l5-4 5 4v4H8V7H4v2H1z"/></svg>
                </div>
                @endif
                <div class="p-2.5">
                    ...
                    <p class="text-[11px] text-indigo-400 mt-1">{{ number_format($property->price, 0, ',', ' ') }} MAD</p>
                    <div class="flex items-center justify-between mt-2">
                        <x-status-badge :status="$property->status" />
                        <a href="{{ route('properties.show', $property) }}"
                           class="text-[10px] text-dark-muted hover:text-dark-text transition-colors">Voir →</a>
                    </div>
                </div>
            </div>
            @empty
            <div class="col-span-2 bg-dark-card border border-dark-border rounded-sm p-8 text-center">
                <p class="text-dark-muted text-xs tracking-wider">Aucun favori sauvegardé</p>
                <a href="{{ route('properties.index') }}" class="inline-block mt-2 text-xs text-indigo-400 hover:underline">
                    Explorer les biens →
                </a>
            </div>
            @endforelse
    ...
    {{-- Mes RDV --}}
    <div>
        <div class="text-[9px] tracking-[.15em] text-dark-dim uppercase mb-2">Mes rendez-vous</div>
        <div class="flex flex-col gap-1.5">
            @forelse($myAppointments as $apt)
            <div class="bg-dark-card2 border border-dark-border rounded-sm p-2.5 flex items-center justify-between">
                <div>
                    <p class="text-[12px] text-dark-text">{{ $apt->property->title }}</p>
                    <p class="text-[10px] text-dark-muted mt-0.5">
                        {{ $apt->date->format('d/m/Y') }} — {{ $apt->time }}
